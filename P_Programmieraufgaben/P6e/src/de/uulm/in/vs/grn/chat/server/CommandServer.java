    ...
    public void handleMessage(CommandClient client, Message message) {
        switch (message.command) {
            case "SEND": {
                ...
                if (message.fields.get("Text").contains("\n") || message.fields.get("Text").contains("\r")) {
                    Debug.println("Command Server Received Message that contains invalid chars");
            ...
            case "BYE": {
                ...
                if (clients.containsKey(client)) {
                    ...
                    eventRelay.sendEvent(username + " has left");
            ...
            case "PING": {
                ...
                pong.fields.put("Date", Util.dateFormat.format(new Date()));
                final StringBuilder usersCSV = new StringBuilder();
                clients.forEach((c, username) -> usersCSV.append(username).append(", "));
                String usersCSVString = usersCSV.toString();
                if (!usersCSVString.equals("")) {
                    usersCSVString = usersCSVString.substring(0, usersCSVString.length() - 2);
                }
                pong.fields.put("Usernames", usersCSVString);
